...
class DataLatteUa {
    ...
    public static function getFolderJs($path){
        ...
        foreach(LatteReflection::getFileList($path, 'js') as $file){
            $response .= file_get_contents(StringUtil::combinePath($path, $file));
        }
        ...
        foreach($folders as $folder){
            ...
            // Compose folder path
            $folderPath = StringUtil::combinePath($path, $folder);
            
            // Scan files in folder
            foreach(LatteReflection::getFileList($folderPath, 'js') as $file){
                $files[] = StringUtil::combinePath($folderPath, $file);
            }
        }
    ...
    private static function getNamespaceFiles($app, $namespace){
        ...
        foreach($files as $i => $file)
            $files[$i] = StringUtil::combinePath($path, $file);
...
//        echo PHP_EOL . " */";
    ...
    public static function dumpRuntimeJs(){
        ...
        readfile(StringUtil::combinePath(DATALATTE_CORE, 'support/js/datalatte-runtime.js'));
...
//            throw new Exception("Cant insert record");
    ...
    private static function getPathForNamespace($app, $namespace){
        ...
        foreach(self::$namespaceDirectories as $dir){
            $possible = StringUtil::combinePath($dir, $namespace);
            $check = StringUtil::combinePath($possible, '.');
            $checked[] = $dir;
...
//        }
    ...
    public static function appTags($app = 'App', $includeCore = true){
        ...
        return array(
            ...
            ->attr("src", StringUtil::combinePath(self::$path , '_core/support/js/jquery-1.7.2.min.js')),
    ...
    public static function runtimeTags(){
        return array(
            tag("script")
            ->attr("type", "text/javascript")
            ->attr("src", StringUtil::combinePath(self::$path , 'support/js/jquery-1.7.2.min.js')),
